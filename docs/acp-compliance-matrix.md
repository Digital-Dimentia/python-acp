...
| 13 | `ext_method` | `_<name>` | **implement, answering `-32601`** | — | Not an ACP method: the router strips a leading `_` and forwards (`acp/router.py:174-178`). It was to host the legacy MCP passthrough during the D4 window; `pyacp-sld.2` declined that — the passthrough addressed the process-wide `--mcp-command` server, and `pyacp-sld.3` deleted both rather than carrying either onto the ACP surface. There are no extensions, so every one is a genuine method-not-found. |
| 14 | `ext_notification` | `_<name>` | **implement** | — | Same mechanism, notification side. Must stay silent on unknown names — the router returns `None` when the handler is absent, and we must not turn an unknown extension into an error. |
