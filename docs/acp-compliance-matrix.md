...
| 13 | `ext_method` | `_<name>` | **implement, answering `-32601`** | — | Not an ACP method: the router strips a leading `_` and forwards (`acp/router.py:174-178`). It was to host the legacy MCP passthrough during the D4 window; `pyacp-sld.2` declined that — the passthrough addresses the process-wide `--mcp-command` server and is deleted with the deprecated surface, not carried onto the ACP one. There are no extensions, so every one is a genuine method-not-found. |
